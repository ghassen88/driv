  ...
  template: `
    <svg:g  *ngFor="let arc of data; trackBy: trackBy">
      ...
      <svg:text
          ...
          <svg:text *ngIf="showTotal" class="label" dy="0.5em" x="0" y="5" text-anchor="middle">
            Total
          </svg:text>
      <svg:g
        ngx-charts-pie-arc
...
export class PieSeriesComponent implements OnChanges,AfterViewInit {
  ...
  getTotal(): number {
    return this.series.map(d => d.value).reduce((sum, d) => sum + d, 0);
  }
  outerArc(): any {
    const factor = 1.5;    
    return arc()
